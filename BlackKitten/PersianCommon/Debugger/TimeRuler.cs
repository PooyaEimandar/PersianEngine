...
namespace Debugger
{
    ...
    public class TimeRuler
    {
        ...
        public TimeRuler(GraphicsDevice GraphicsDevice)
        {
#if TRACE
        ...
        public void StartFrame()
        {
            ...
            lock (this)
            {
                ...
                if (Active && (1 < count && count < MaxSampleFrames))
                    return;
        ...
        public void Draw(SpriteBatch spriteBatch, SpriteFont font, Texture2D WhiteTexture)
        {
            Draw(spriteBatch, font, WhiteTexture, position, Width);
        }

        [Conditional("TRACE")]
        public void Draw(SpriteBatch spriteBatch, SpriteFont font, Texture2D WhiteTexture, Vector2 position, int width)
        {
#if TRACE
            // Reset update count.
            Interlocked.Exchange(ref updateCount, 0);
            ...
            foreach (MarkerCollection bar in prevLog.Bars)
            {
                ...
                if (bar.MarkCount > 0)
                {
                    for (int j = 0; j < bar.MarkCount; ++j)
                    {
                        ...
                        spriteBatch.Draw(WhiteTexture, rc, bar.Markers[j].Color);
                    }
                }
            ...
            for (float t = 1.0f; t < sampleSpan; t += 1.0f)
            {
                ...
                spriteBatch.Draw(WhiteTexture, rc, Color.Gray);
            }
            ...
            for (int i = 0; i <= sampleFrames; ++i)
            {
                ...
                spriteBatch.Draw(WhiteTexture, rc, Color.White);
            }
                ...
                foreach (MarkerInfo markerInfo in markers)
                {
                    for (int i = 0; i < MaxBars; ++i)
                    {
                        if (markerInfo.Logs[i].Initialized)
                        {
                            ...
                            rc2.Y = y + 1;
                            spriteBatch.Draw(WhiteTexture, rc, Color.White);
                            spriteBatch.Draw(WhiteTexture, rc2, markerInfo.Logs[i].Color);
